...
import {createVideoTypeRegExp, insertVideoTag} from './utils/common';
import VideoUploaderCommand from "./commands/videouploadercommand";
...
export default class VideoUploaderEditing extends Plugin {
    ...
    init() {
        ...
        const fileTypes = createVideoTypeRegExp(editor.config.get('video.upload.types'));
        ...
        schema.extend('$text', {allowAttributes: ['videoId', 'videoUrl', 'uploadStatus']});
        ...
        doc.on('change', () => {
            ...
            for (const entry of changes) {
                ...
                if (entry.type == 'insert') {
                    ...
                    for (const videoElement of getVideoShortcodes(editor, item)) {
        ...
        this.on('uploadVideoComplete', (evt, {videoElement, data}) => {
    ...
    _readAndUpload(loader, videoElement) {
        ...
        return loader.read()
            ...
            .then(data => {
                model.enqueueChange('transparent', writer => {
                    writer.setAttributes({uploadStatus: 'complete', videoUrl: data.default}, videoElement);
